...
The site intentionally uses semantic HTML, a portable `tokens.css`, one shared page stylesheet, and local progressive-enhancement JavaScript. Anime.js 4.5.0 draws four finite SVG project traces. ScrollMagic 2.0.8, Lottie Web 5.13.0, and GSAP 3.15.0 run the home page's scene, semantic-glyph, and state transitions; VueUse Motion and React Spring patterns are adapted in vanilla JavaScript so the static site does not ship Vue or React solely for animation. Three.js 0.185.1 loads the home Heroic Alpha Station from eight compressed same-origin GLBs. The site still requires no package manager or build step at runtime and makes no third-party runtime requests. Sources and licenses are recorded in `THIRD_PARTY_NOTICES.md`.
...
Motion is semantic progressive enhancement for the Narrative Workflow and Long Document structures. A 20-effect score uses four bounded patterns from each library named in the referenced video: ScrollMagic scene/scrub behavior; VueUse Motion visible, parallax, and press patterns; React Spring trail, wave, dock, and node responses; four Lottie workflow glyphs; and four GSAP entrance/state transitions. The Hero offers two finite, manually started replays: PASS resolves all six gates and opens a human `release / revise / reject` decision; BLOCK stops at G3 `residue_map`, writes a receipt, and keeps release disabled. The active brass trace is written only after the gate verdict. Drag and arrow-key rotation are limited to ±12 degrees; particle displacement remains local and springs back. Each project uses native scroll to select one project-specific step, draw its entering SVG edge, update a compact status readout, and rewrite an `event / owner / outcome` receipt. `prefers-reduced-motion` keeps both replays and all controls usable while removing interpolation. There is no scroll hijacking, free orbit, continuous loop, cursor replacement, typing effect, or background timer.
...
4. methods and motion credits, research context, profile, and contact.
